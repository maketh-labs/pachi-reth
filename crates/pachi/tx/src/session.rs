...
/// A session-key delegated transaction (type 0x04).
///
/// The session key signs the transaction, but `msg.sender` is set to the authorizer
/// (main wallet) during execution. Gas is paid by the authorizer.
#[derive(Clone, Debug, Default, PartialEq, Eq, Hash)]
pub struct SessionTx {
    /// Chain ID (EIP-155).
    pub chain_id: ChainId,
    /// Session key nonce (NOT authorizer nonce).
    pub nonce: u64,
...
impl RlpEcdsaEncodableTx for SessionTx {
...
impl RlpEcdsaDecodableTx for SessionTx {
    const DEFAULT_TX_TYPE: u8 = PachiTxType::Session as u8;
...
impl Transaction for SessionTx {
...
impl Typed2718 for SessionTx {
    fn ty(&self) -> u8 {
        PachiTxType::Session as u8
    }
}

impl IsTyped2718 for SessionTx {
    fn is_type(type_id: u8) -> bool {
        type_id == PachiTxType::Session as u8
    }
}

impl SignableTransaction<Signature> for SessionTx {
